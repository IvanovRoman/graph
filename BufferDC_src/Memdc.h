...
public:
	...
	BOOL	IsDirty();
	// returns TRUE if the buffer has not been initialized and if it should be filled by drawing
	// operations

	void	Dirty();
	// set dirty flag of the buffer, all subsequent IsDirty() will return TRUE after this call

	void	EndModify();
	// tell the buffer everything is drawn, all subsequent IsDirty() will return FALSE after
	// this call. The buffer can be used now
...
// functions used by CMemDC:
	...
	CRect   GetSizeBuffer() const;
	// get the size of the bitmap

	CBitmap *GetBitmap();
	// get the bitmap assoziated with the buffer

	void    OnScroll( int dx, int dy);
	// does do the same as CWnd::ScrollWindow()
...
private:
	...
	BOOL    m_LowMemory;
	// TRUE when the creation of a bitmap has failed. A flag for low system resources
};
...
private:
	...
	CBitmap			*m_oldBitmap;
	// bitmap originaly found in pBufferMemDC

	CDC				*m_pDC;
	// The CDC passed in the constructor

	CRect			m_ClipRect;
	// The clipping rect we are using

	BOOL			m_bMemDC;
	// TRUE if the CDC is a memory DC

	DWORD			m_LastCopyToScreen;
	// GetTickCount() of last CopyToScreen
};
